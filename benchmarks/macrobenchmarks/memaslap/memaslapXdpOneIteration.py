...
def run_memaslap_and_capture_output():
    ...
    memaslap_cmd = [
        "memaslap",
        "-s", "192.168.0.55:11211",
        "-T", "2",
        "-c", "1024",
        "-n", "1",
        "-t", "120s",
        "--stat_freq=20s"  # Adjust stat_freq as per your requirement
    ]
...
def main():
    # Run memaslap and capture output
    ...
    if output:
        ...
        plot_tps_with_error_bars(ax1, get_data['interval'], get_data['tps'], label='Get')

        # Plot Set TPS with error bars
        plot_tps_with_error_bars(ax2, set_data['interval'], set_data['tps'], label='Set')

        # Plot Get Latency with error bars
        plot_latency_with_error_bars(ax3, get_data['interval'], get_data['latency_avg'], get_data['latency_stddev'], label='Get')

        # Plot Set Latency with error bars
        plot_latency_with_error_bars(ax4, set_data['interval'], set_data['latency_avg'], set_data['latency_stddev'], label='Set')

        # Save the figure as PNG
        plt.tight_layout()
        plt.savefig('statistics_plots_xdp.png')
        plt.close()
