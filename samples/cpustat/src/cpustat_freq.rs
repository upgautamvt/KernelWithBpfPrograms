...
pub extern "C" fn _start(ctx: &cpu_args) -> u32 {
    let (pstate_duration, my_map): (u32, u32) = (1, 2);
    let (mut pts, cstate, pstate, cur_ts, delta): (u64, u64, u64, u64, u64);
	let (cskey, ptkey, pskey, vkey, cpu, pstate_idx): (u32, u32, u32, u32, u32, u32);
    ...
    bpf_map_update_elem(my_map, pskey, pstate, stub::BPF_ANY);
