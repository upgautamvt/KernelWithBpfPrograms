...
#define EXE "./target/debug/bmc_kern"
...
int main(int argc, char *argv[])
{
	...
	rx_prog_fd = iu_obj_get_prog(base_fd, "bmc_rx_filter");

	if (rx_prog_fd < 0) {
		fprintf(stderr, "bmc_rx_filter not found\n");
		exit(1);
	}

	tx_prog_fd = iu_obj_get_prog(base_fd, "bmc_tx_filter");

	if (tx_prog_fd < 0) {
		fprintf(stderr, "bmc_tx_filter not found\n");
		exit(1);
	}
